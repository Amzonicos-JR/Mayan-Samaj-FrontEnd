...
export const TablePaymentMethod = () => {
  ...
  const getPayments = async (id) => {
    try {
      const { data } = await axios.get(`http://localhost:3000/paymentMethod/get`, {
        headers: headers,
      });
      console.log(data);
      setPatments(data.paymentMethods);
    } catch (err) {
      console.log(err);
  ...
  return (
    <>
      ...
      <table className="table table-striped">
        ...
        <tbody>
          {payments?.map(({ _id, name, paymentMethod_type }, index) => {
            return (
              <tr className="text-center" key={index}>
                ...
                ></PaymentMethod>
                <td>
                  <Link to={`/dash/payments/updateP/${_id}`}>
                    <svg
